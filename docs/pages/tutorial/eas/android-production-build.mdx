...
const ChildGrowthTracker = () => {
  ...
  useEffect(() => {
    saveData();
  }, [name, age, height, weight, milestones, profileImage, heightData, weightData]);

  const saveData = async () => {
    try {
      const data = { name, age, height, weight, milestones, profileImage, heightData, weightData };
  ...
  const loadData = async () => {
    try {
      const data = await AsyncStorage.getItem("childData");
      if (data) {
        ...
        setHeightData(parsedData.heightData || []);
  ...
  const addMilestone = () => {
    if (newMilestone.trim() === "") {
      Alert.alert("Error", "Milestone cannot be empty");
      return;
    }
  ...
  const updateGrowthData = () => {
    setHeightData([...heightData, parseFloat(height)]);
    setWeightData([...weightData, parseFloat(weight)]);
  };
  ...
  return (
    <ScrollView style={{ padding: 20 }}>
      ...
      {profileImage && <Image source={{ uri: profileImage }} style={{ width: 100, height: 100, borderRadius: 50, marginBottom: 10 }} />}
      <Button title="Upload Profile Image" onPress={pickImage} />
      <Text>Name:</Text>
      <TextInput value={name} onChangeText={setName} style={{ borderWidth: 1, marginBottom: 10, padding: 5 }} placeholder="Enter child's name" />
      <Text>Age:</Text>
      <TextInput value={age} onChangeText={setAge} keyboardType="numeric" style={{ borderWidth: 1, marginBottom: 10, padding: 5 }} placeholder="Enter age in years" />
      <Text>Height (cm):</Text>
      <TextInput value={height} onChangeText={setHeight} keyboardType="numeric" style={{ borderWidth: 1, marginBottom: 10, padding: 5 }} placeholder="Enter height in cm" />
      <Text>Weight (kg):</Text>
      <TextInput value={weight} onChangeText={setWeight} keyboardType="numeric" style={{ borderWidth: 1, marginBottom: 10, padding: 5 }} placeholder="Enter weight in kg" />
      <Button title="Update Growth Data" onPress={updateGrowthData} />
      <Text>Milestones:</Text>
      <TextInput value={newMilestone} onChangeText={setNewMilestone} style={{ borderWidth: 1, marginBottom: 10, padding: 5 }} placeholder="Enter a milestone" />
      <Button title="Add Milestone" onPress={addMilestone} />
      <View style={{ marginTop: 20 }}>
        {milestones.map((milestone, index) => (
          <View key={index} style={{ flexDirection: "row", alignItems: "center", marginBottom: 5, backgroundColor: "#f0f0f0", padding: 5 }}>
            <Text style={{ flex: 1 }}>{milestone}</Text>
      ...
      {heightData.length > 0 && (
        <View style={{ marginTop: 20 }}>
          <Text style={{ fontSize: 18, fontWeight: "bold" }}>Growth Progress</Text>
          <LineChart
            data={{
              labels: heightData.map((_, i) => `Entry ${i + 1}`),
              datasets: [
                { data: heightData, color: () => "blue" },
                { data: weightData, color: () => "red" },
              ],
            }}
